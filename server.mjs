// server.mjs (ESM) — API + Static + Azure PostgreSQL
...
// -------------------------------------------------------------------
// 1) Obtener y parsear la Connection String de Azure App Service
//    Soporta: POSTGRESQLCONNSTR_* (Type=PostgreSQL) y CUSTOMCONNSTR_* (Type=Custom)
//    Fallback: variables discretas DB_HOST/DB_USER/DB_PASSWORD/DB_NAME/DB_PORT/DB_SSL
// -------------------------------------------------------------------
...
if (explicitEnv.host && explicitEnv.database && explicitEnv.user) {
  ...
  if (!cs) {
    console.error("No DB env vars and no App Service Connection String found");
    process.exit(1);
  }
...
console.log("PG config →", {
  host: poolConfig.host, port: poolConfig.port, database: poolConfig.database, ssl: !!poolConfig.ssl
});

const pool = new Pool(poolConfig);

// -------------------------------------------------------------------
// 2) Esquema mínimo (idempotente)
// -------------------------------------------------------------------
...
app.get("/api/projects", async (_req, res) => {
  try {
    ...
    console.error(e); res.status(500).json({ error: "Error getting projects" });
  }
});
...
app.post("/api/projects", async (req, res) => {
  try {
    const id = uuidv4();
    const {
      nombre, numero_oportunidad, pais, consultor, monto_oportunidad,
    ...
    console.error(e); res.status(500).json({ error: "Error creating project" });
  }
});
...
app.put("/api/projects/:id", async (req, res) => {
  try {
    ...
    const fields = [
      "nombre","numero_oportunidad","pais","consultor","monto_oportunidad","terminado",
      "client_name","pm","planned_hours","executed_hours","hourly_rate","start_date","end_date"
    ];
    const updates = [];
    const values = [];
    ...
    console.error(e); res.status(500).json({ error: "Error updating project" });
  }
});
...
app.delete("/api/projects/:id", async (req, res) => {
  try {
    ...
    console.error(e); res.status(500).json({ error: "Error deleting project" });
  }
});
...
app.post("/api/projects/:id/observations", async (req, res) => {
  try {
    ...
    console.error(e); res.status(500).json({ error: "Error adding observation" });
  }
});
...
app.delete("/api/projects/:id/observations/:obsId", async (req, res) => {
  try {
    ...
    console.error(e); res.status(500).json({ error: "Error deleting observation" });
  }
});
...
app.get("/api/visits", async (_req, res) => {
  try {
    ...
    console.error(e); res.status(500).json({ error: "Error getting visits" });
  }
});
